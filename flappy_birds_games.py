...
def draw_floor():
    ...
    screen.blit(floor_normal, (floor_x_pos,900))
    screen.blit(floor_normal, (floor_x_pos +576,900))


def create_pipe():

    random_pipe_pos = random.choice(pipe_height)
    bottom_pipe = pipe_normal.get_rect(midtop =(700,random_pipe_pos))
    top_pipe = pipe_normal.get_rect(midbottom =(700,random_pipe_pos -300))
...
def draw_pipes(pipes): 
    ...
    for pipe in pipes:
        if pipe.bottom >= 1024:
            screen.blit(pipe_normal, pipe)
        else:
            flip_pipe = pygame.transform.flip(pipe_normal, False, True)
...
def update_score(score, high_score):
    #updating the score and keeping track of the highest score
    if score>high_score:
        high_score = score
...
def pipe_score_check():
    #Counting the score only after successfully crossing a pipe
...
bg_normal = pygame.transform.scale2x(bg_surface)

#floor surface
floor_surface = pygame.image.load('sprites/base.png').convert()
floor_normal = pygame.transform.scale2x(floor_surface)
floor_x_pos = 0
...
pipe_normal = pygame.transform.scale2x(pipe_surface)
pipe_list = []
SPAWNPIPE = pygame.USEREVENT
pygame.time.set_timer(SPAWNPIPE,1200)
pipe_height = [400, 600, 500, 550, 450, 750, 800]



game_over_surface = pygame.transform.scale2x(pygame.image.load('sprites/message.png').convert_alpha())
game_over_rect = game_over_surface.get_rect(center = (258, 512))
...
while True:
    ...
    if game_active:
        #while collision is not happening
    ...
    clock.tick(120) #keeping the frame rate maxiumum under 120